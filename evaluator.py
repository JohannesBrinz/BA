...
plt.text(-110, 0.03, '#real eigenvalues:   ' + str(len(real_lambda_N3.real)))
plt.text(-110, 0.028,  '$N^2$ = 9')
...
plt.xlabel('$\lambda$', fontsize = 13)
plt.ylabel('probability distribution', fontsize = 13)
plt.text(-90, 0.03, '#real eigenvalues:   ' + str(len(real_lambda_N6.real)))
plt.text(-90, 0.028,  '$N^2$ = 36')
...
plt.xlabel('$|\lambda_i - \lambda_{i+1}|$', fontsize = 13)
plt.ylabel('probability distribution', fontsize = 13)
plt.text(10, 0.8, '#eigenvalues:   ' + str(len(dif6)))
plt.text(10, 0.7,  '$N^2$ = 36')
...
mean_lambda = pd.DataFrame([float(7.0), float(17.0), float(27.0), float(45.0), float(65.0)], columns = ["value"])
N = pd.DataFrame([4, 5, 6, 7, 8], columns = ["value"])
params, params_cov = optimize.curve_fit(lin, N["value"], mean_lambda["value"])
print(params)
plt.errorbar(N, mean_lambda, fmt = "D")
plt.errorbar(np.linspace(4,8, 1000), lin(np.linspace(4,8, 1000), params[0], params[1]), fmt = "--", color = "black")
